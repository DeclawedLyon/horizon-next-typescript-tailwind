...
const apiKey = 'AIzaSyBunH5UFVpp4hyUSAKK7kFUe5MvGHb5us4'
...
function MapWindow(props) {
  ...
  const formatCutList = (cutArr) => {
    const formattedCuts = cutArr.map((cutObj, x) => {
      let UTCCutResult = new Date(cutObj.lastCutDate)
      // console.log(UTCCutResult)
      const cutResult = dateTest(UTCCutResult).result
      let UTCTrimResult = new Date(cutObj.lastTrimDate)
      // console.log(UTCTrimResult)
      const trimResult = dateTest(UTCTrimResult).result
      let polygonOptions = {
